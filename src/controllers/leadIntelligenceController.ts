...
import { generateLeadsCsv, generateBatchLeadsCsv } from '../services/leadIntelligence/exportService';
...
export async function processUrl(req: Request, res: Response): Promise<void> {
  try {
    ...
    const name = sourceName?.trim() || `Website Research (${new URL(url.startsWith('http') ? url : 'https://' + url).hostname})`;
    const workspaceId = await resolveUserWorkspaceId(user.userId);
...
export async function getLeads(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Counts by status
    const [total, uniqueCount, duplicateCount, possibleCount, leads] = await Promise.all([
...
export async function exportBatchCsv(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Batch-scoped export: fetch all records belonging to this batch
    const leads = await prisma.leadIntelligenceLead.findMany({
    ...
    const csvData = generateBatchLeadsCsv(leads, batch.sourceUrl);
