...
export const personalizeContact = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await PersonalizationService.generateForContact(id, !!force);
...
export const editPersonalization = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const updated = await PersonalizationService.updateManualPersonalization(id, personalization);
    res.status(200).json({
      success: true,
